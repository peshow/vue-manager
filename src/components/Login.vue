    ...
    <div class="login-wrapper">
      <el-form ref="form" :rules="rules" :model="form" label-width="80px">
        ...
        <el-form-item>
          ...
          <p class="error-message" v-if="exists">您输入的账户密码错误</p>
      </el-form>
    </div>
  </div>
</template>
  ...
  methods: {
    onSubmit (formName) {
      ...
      self.$refs[formName].validate((valid) => {
        if (valid) {
          self.$axios.get(`http://172.16.20.215:9999/api/login/get?id=${self.form.username}&pass=${self.form.password}`)
          .then(function (rest) {
            let result = rest.data.rest
