  ...
  <div>
    ...
    <el-table :data="hostData" border style="width: 100%" v-loading="loading" element-loading-text="拼命加载中"  @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" width="140">
        <template scope="scope">
          <el-button size="small" :disabled="scope.row.on" type="success" @click="handleAction(scope.$index, scope.row, 'start')">启用</el-button> 
          <el-button size="small" :disabled="scope.row.off" type="danger" @click="handleAction(scope.$index, scope.row, 'stop')">禁用</el-button>
        </template>
      </el-table-column>
    </el-table>
  </div>
